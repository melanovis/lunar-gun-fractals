...
dims_x = [center_x-half_width,center_x+half_width];
dims_y = [center_y-half_width/2,center_y+half_width/2];

long_range = linspace(dims_x(1),dims_x(2),res(2));
lat_range = linspace(dims_y(2),dims_y(1),res(1));
...
% save("endstate_mat.mat","endstate_struct")
% 
% load("endstate_mat.mat")
...
%endstate_struct = circshift(endstate_struct, [0,round(width(endstate_struct)*0.25)]);
...
imwrite(frame_coloured, "frac_large.png");
...
wm = image(plotmap ,'xdata',[min(long_range),max(long_range)],'ydata',[max(lat_range),min(lat_range)]);
