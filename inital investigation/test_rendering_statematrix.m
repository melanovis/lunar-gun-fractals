...
parfor n=1:numel(long_range)
    for m=1:lat_range_length
        ...
        return_struct = run_transit(lunar_start_statematrix, earth_start_statematrix, scene_time, launch_long, launch_lat, launch_azimuth, launch_elevation, eject_v, t_start);
        test_frac(m,n) = return_struct;
        test_check(m,n) = logical(all(return_struct.check_matrix));
        test_time(m,n) = return_struct.transit_time;
    end
    fprintf("- %3.2f.\n",n/numel(long_range))
end
disp(toc)

% save("test.mat","test_frac")
% 
% load("test.mat")
...
time_range = linspace(0,scene_time,100);
...
for ind_t = 1:numel(time_range)
    ...
    frame_coloured = uint8(zeros([size(test_frac),3]));
    %ind_t = 100;
    
    for n=1:height(test_frac)
        for m=1:width(test_frac)
            pixel_statematrix = test_frac(n,m).transit_statematrix;
            pixel_lifespan = test_frac(n,m).transit_time;
    ...
    if record_video
        ...
        frame = getframe(gcf);
        for n=1:3
            frame_new(:,:,n) = uint8(imresize(squeeze(frame.cdata(:,:,n)), [nan, 1920]));
        end
...
if record_video
    close(v);
    sound(sin(2*pi*400*(0:1/14400:0.5)), 14400);
end
