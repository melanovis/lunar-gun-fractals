function return_struct = run_transit(lunar_start_statematrix, earth_start_statematrix, scene_time, launch_long, launch_lat, launch_azimuth, launch_elevation, eject_v, t_start, return_earthmoon_statemat)
    ...
    if ~return_earthmoon_statemat
        ode_settings = odeset('RelTol', error_tolerance,'Events', @boundary_check);
    else
        ode_settings = odeset('RelTol', error_tolerance);
    end
    ...
    total_statematrix_original = total_transit_statematrix;
    ...
    if return_earthmoon_statemat
        earthluna_statematrix = [total_statematrix_original(:,1), total_statematrix_original(:,8:end)];
        earthluna_statematrix(:,1) = earthluna_statematrix(:,1) - earthluna_statematrix(1,1);
    end

    total_transit_statematrix(:,5:end) = [];
    total_transit_statematrix = round(total_transit_statematrix,2);

    total_transit_statematrix(:,1) = total_transit_statematrix(:,1) - total_transit_statematrix(1,1);

    return_struct = struct();
    return_struct = setfield(return_struct,"transit_statematrix",single(total_transit_statematrix));
    ...
    if return_earthmoon_statemat
        return_struct = setfield(return_struct,"earth_moon_statematrix",single(earthluna_statematrix));
    end
