function return_struct = run_transit(lunar_start_statematrix, earth_start_statematrix, scene_time, launch_long, launch_lat, launch_azimuth, launch_elevation, eject_v, t_start, time_samples)
    ...
    ER3BP_timespan = linspace(0,scene_time,time_samples);
    ...
    total_transit_statematrix(:,8:end) = [];

    total_transit_statematrix(:,1) = total_transit_statematrix(:,1) - total_transit_statematrix(1,1);

    ind_sigchange = [true; abs(diff(total_transit_statematrix(:,1))) > 5];
    total_transit_statematrix = total_transit_statematrix(ind_sigchange,:);

    [~, ind_unique] = unique(total_transit_statematrix(:, 1), 'stable');
    total_transit_statematrix = total_transit_statematrix(ind_unique,:);
