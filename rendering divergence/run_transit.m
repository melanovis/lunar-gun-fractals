function return_struct = run_transit(lunar_start_statematrix, earth_start_statematrix, scene_time, launch_long, launch_lat, launch_azimuth, launch_elevation, eject_v, t_start)
    ...
    ER3BP_timespan = linspace(0,scene_time,600);
    ...
    total_transit_statematrix = single(total_transit_statematrix);

    ind_sigchange = [true; abs(diff(total_transit_statematrix(:,1))) > 10]; %only keeping changes by 10 sec or greater
